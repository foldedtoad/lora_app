...
#include "LoRa_MacClassBConfig.h"
...
/*
 * LoRa_Mac Class B Context structure for NVM parameters
 * related to ping slots
 */
typedef struct sLoRa_MacClassBPingSlotNvmCtx
{
     ...
     */
    int8_t Datarate;
} LoRa_MacClassBPingSlotNvmCtx_t;

/*
 * LoRa_Mac Class B Context structure for NVM parameters
 * related to beaconing
 */
typedef struct sLoRa_MacClassBBeaconNvmCtx
{
     ...
     */
    uint32_t Frequency;
} LoRa_MacClassBBeaconNvmCtx_t;

/*
 * LoRa_Mac Class B Context structure
 */
typedef struct sLoRa_MacClassBNvmCtx
{
    /*!
    * Class B ping slot context
    */
    LoRa_MacClassBPingSlotNvmCtx_t PingSlotCtx;
    /*!
    * Class B beacon context
    */
    LoRa_MacClassBBeaconNvmCtx_t BeaconCtx;
} LoRa_MacClassBNvmCtx_t;

/*
 * LoRa_Mac Class B Context structure
 */
typedef struct sLoRa_MacClassBCtx
{
    /*!
    * Class B ping slot context
    */
    ...
    */
    LoRa_MacClassBCallback_t LoRa_MacClassBCallbacks;
    /*!
    * Data structure which holds the parameters which needs to be set
    * in class b operation.
    */
    LoRa_MacClassBParams_t LoRa_MacClassBParams;
    /*
     * Callback function to notify the upper layer about context change
     */
    LoRa_MacClassBNvmEvent LoRa_MacClassBNvmEvent;
    /*!
    * Non-volatile module context.
    */
    LoRa_MacClassBNvmCtx_t* NvmCtx;
} LoRa_MacClassBCtx_t;

/*!
 * Defines the LoRa_Mac radio events status
 */
typedef union uLoRa_MacClassBEvents
{
    uint32_t Value;
    struct sEvents
    {
...
}LoRa_MacClassBEvents_t;

LoRa_MacClassBEvents_t LoRa_MacClassBEvents = { .Value = 0 };

/*
 * Non-volatile module context.
 */
static LoRa_MacClassBNvmCtx_t NvmCtx;

/*
 * Module context.
 */
static LoRa_MacClassBCtx_t Ctx;
...
static uint32_t CalcDownlinkFrequency( uint8_t channel )
{
    ...
    phyParam = RegionGetPhyParam( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &getPhy );
...
static void RxBeaconSetup( TimerTime_t rxTime, bool activateDefaultChannel )
{
    ...
    if( ( Ctx.BeaconCtx.Ctrl.BeaconAcquired == 1 ) || ( Ctx.BeaconCtx.Ctrl.AcquisitionPending == 1 ) )
    {
        ...
        RegionComputeRxWindowParameters( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion,
                                        ( int8_t )phyParam.Value, // datarate
                                        Ctx.LoRa_MacClassBParams.LoRa_MacParams->MinRxSymbols,
                                        Ctx.LoRa_MacClassBParams.LoRa_MacParams->SystemMaxRxError,
                                        &beaconRxConfig );
    ...
    RegionRxBeaconSetup( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &rxBeaconSetup, &Ctx.LoRa_MacClassBParams.McpsIndication->RxDatarate );

    Ctx.LoRa_MacClassBParams.MlmeIndication->BeaconInfo.Frequency = frequency;
    Ctx.LoRa_MacClassBParams.MlmeIndication->BeaconInfo.Datarate = Ctx.LoRa_MacClassBParams.McpsIndication->RxDatarate;
}
...
static void GetTemperatureLevel( LoRa_MacClassBCallback_t *callbacks, BeaconContext_t *beaconCtx )
{
...
static void InitClassB( void )
{
    ...
    // Init events
    LoRa_MacClassBEvents.Value = 0;

    // Init variables to default
    memset1( ( uint8_t* ) &NvmCtx, 0, sizeof( LoRa_MacClassBNvmCtx_t ) );
...
static void InitClassBDefaults( void )
{
    ...
    LoRa_MacClassBBeaconNvmCtx_t beaconCtx = Ctx.NvmCtx->BeaconCtx;
    LoRa_MacClassBPingSlotNvmCtx_t pingSlotCtx = Ctx.NvmCtx->PingSlotCtx;
...
static void IndicateBeaconStatus( LoRa_MacEventInfoStatus_t status )
{
    if( Ctx.BeaconCtx.Ctrl.ResumeBeaconing == 0 )
    {
        Ctx.LoRa_MacClassBParams.MlmeIndication->MlmeIndication = MLME_BEACON;
        Ctx.LoRa_MacClassBParams.MlmeIndication->Status = status;
        Ctx.LoRa_MacClassBParams.LoRa_MacFlags->Bits.MlmeInd = 1;

        Ctx.LoRa_MacClassBParams.LoRa_MacFlags->Bits.MacDone = 1;
    }
...
static TimerTime_t UpdateBeaconState( LoRa_MacEventInfoStatus_t status,
...
{
    if( Ctx.LoRa_MacClassBNvmEvent != NULL )
    {
        Ctx.LoRa_MacClassBNvmEvent( );
    }
}

#endif // LORA_MAC_CLASSB_ENABLED
...
#ifdef LORA_MAC_CLASSB_ENABLED
    // Store callbacks
    Ctx.LoRa_MacClassBCallbacks = *callbacks;

    // Store parameter pointers
    Ctx.LoRa_MacClassBParams = *classBParams;

    // Assign non-volatile context
    Ctx.NvmCtx = &NvmCtx;

    // Assign callback
    Ctx.LoRa_MacClassBNvmEvent = classBNvmCtxChanged;

    // Initialize timers
    TimerInit( &Ctx.BeaconTimer, LoRa_MacClassBBeaconTimerEvent );
    TimerInit( &Ctx.PingSlotTimer, LoRa_MacClassBPingSlotTimerEvent );
    TimerInit( &Ctx.MulticastSlotTimer, LoRa_MacClassBMulticastSlotTimerEvent );
...
void LoRa_MacClassBSetBeaconState( BeaconState_t beaconState )
{
    ...
    if( beaconState == BEACON_STATE_ACQUISITION )
    {
        ...
        if( ( Ctx.BeaconCtx.Ctrl.BeaconDelaySet == 1 ) &&
            ( LoRa_MacClassBIsAcquisitionPending( ) == false ) )
        {
...
bool LoRa_MacClassBIsAcquisitionInProgress( void )
{
    ...
    if( LoRa_MacClassBIsAcquisitionPending( ) == true )
    {
...
void LoRa_MacClassBBeaconTimerEvent( void* context )
{
    ...
    if( Ctx.LoRa_MacClassBCallbacks.MacProcessNotify != NULL )
    {
        Ctx.LoRa_MacClassBCallbacks.MacProcessNotify( );
    }
#endif // LORA_MAC_CLASSB_ENABLED
}

#ifdef LORA_MAC_CLASSB_ENABLED
static void LoRa_MacClassBProcessBeacon( void )
{
    ...
    switch( Ctx.BeaconState )
    {
        ...
        case BEACON_STATE_REACQUISITION:
        {
            ...
            {
                // Handle beacon miss
                beaconEventTime = UpdateBeaconState( LORA_MAC_EVENT_INFO_STATUS_BEACON_LOST,
        ...
        case BEACON_STATE_LOCKED:
        {
            ...
            beaconEventTime = UpdateBeaconState( LORA_MAC_EVENT_INFO_STATUS_BEACON_LOCKED,
                                                 0, currentTime );

            // Setup the MLME confirm for the MLME_BEACON_ACQUISITION
            if( Ctx.LoRa_MacClassBParams.LoRa_MacFlags->Bits.MlmeReq == 1 )
            {
                if( LoRa_MacConfirmQueueIsCmdActive( MLME_BEACON_ACQUISITION ) == true )
                {
                    LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_OK, MLME_BEACON_ACQUISITION );
                    Ctx.LoRa_MacClassBParams.MlmeConfirm->TxTimeOnAir = 0;
                }
            }
        ...
        case BEACON_STATE_LOST:
        {
            ...
            if( Ctx.LoRa_MacClassBParams.LoRa_MacFlags->Bits.MlmeReq == 1 )
            {
                if( LoRa_MacConfirmQueueIsCmdActive( MLME_BEACON_ACQUISITION ) == true )
                {
                    LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_BEACON_NOT_FOUND, MLME_BEACON_ACQUISITION );
                }
            }
            else
            {
                Ctx.LoRa_MacClassBParams.MlmeIndication->MlmeIndication = MLME_BEACON_LOST;
                Ctx.LoRa_MacClassBParams.MlmeIndication->Status = LORA_MAC_EVENT_INFO_STATUS_OK;
                Ctx.LoRa_MacClassBParams.LoRa_MacFlags->Bits.MlmeInd = 1;
            }
...
{
#ifdef LORA_MAC_CLASSB_ENABLED
    LoRa_MacClassBEvents.Events.PingSlot = 1;
...
#ifdef LORA_MAC_CLASSB_ENABLED
static void LoRa_MacClassBProcessPingSlot( void )
{
    ...
    switch( Ctx.PingSlotState )
    {
        case PINGSLOT_STATE_CALC_PING_OFFSET:
        {
            ComputePingOffset( Ctx.BeaconCtx.BeaconTime.Seconds,
                               *Ctx.LoRa_MacClassBParams.LoRa_MacDevAddr,
        ...
        case PINGSLOT_STATE_IDLE:
        {
            ...
            {
                // Restore floor plan
                frequency = CalcDownlinkChannelAndFrequency( *Ctx.LoRa_MacClassBParams.LoRa_MacDevAddr, Ctx.BeaconCtx.BeaconTime.Seconds, CLASSB_BEACON_INTERVAL );
            }
            ...
            if( Ctx.MulticastSlotState != PINGSLOT_STATE_RX )
            {
                ...
                pingSlotRxConfig.DownlinkDwellTime = Ctx.LoRa_MacClassBParams.LoRa_MacParams->DownlinkDwellTime;
                pingSlotRxConfig.RepeaterSupport = Ctx.LoRa_MacClassBParams.LoRa_MacParams->RepeaterSupport;
                ...
                RegionRxConfig( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &pingSlotRxConfig, ( int8_t* )&Ctx.LoRa_MacClassBParams.McpsIndication->RxDatarate );

                if( pingSlotRxConfig.RxContinuous == false )
                {
                    Radio.Rx( Ctx.LoRa_MacClassBParams.LoRa_MacParams->MaxRxWindow );
                }
                else
                {
...
{
#ifdef LORA_MAC_CLASSB_ENABLED
    LoRa_MacClassBEvents.Events.MulticastSlot = 1;
...
#ifdef LORA_MAC_CLASSB_ENABLED
static void LoRa_MacClassBProcessMulticastSlot( void )
{
    static RxConfigParams_t multicastSlotRxConfig;
    TimerTime_t multicastSlotTime = 0;
    TimerTime_t slotTime = 0;
    MulticastCtx_t *cur = Ctx.LoRa_MacClassBParams.MulticastChannels;
    ...
    switch( Ctx.MulticastSlotState )
    {
        ...
        case PINGSLOT_STATE_SET_TIMER:
        {
            cur = Ctx.LoRa_MacClassBParams.MulticastChannels;
        ...
        case PINGSLOT_STATE_IDLE:
        {
            ...
            multicastSlotRxConfig.DownlinkDwellTime = Ctx.LoRa_MacClassBParams.LoRa_MacParams->DownlinkDwellTime;
            multicastSlotRxConfig.RepeaterSupport = Ctx.LoRa_MacClassBParams.LoRa_MacParams->RepeaterSupport;
            ...
            RegionRxConfig( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &multicastSlotRxConfig, ( int8_t* )&Ctx.LoRa_MacClassBParams.McpsIndication->RxDatarate );
    ...
    if( ( Ctx.BeaconState == BEACON_STATE_RX ) || ( Ctx.BeaconCtx.Ctrl.AcquisitionPending == 1 ) )
    {
        if( size == phyParam.BeaconFormat.BeaconSize )
        {
            ...
            if( crc0 == beaconCrc0 )
            {
                ...
                Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
                Ctx.LoRa_MacClassBParams.MlmeIndication->BeaconInfo.Time = Ctx.BeaconCtx.BeaconTime;
            ...
            if( crc1 == beaconCrc1 )
            {
                ...
                Ctx.LoRa_MacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.InfoDesc = payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2];
                memcpy1( Ctx.LoRa_MacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.Info, &payload[phyParam.BeaconFormat.Rfu1Size + 4 + 2 + 1], 6 );
            }
...
void LoRa_MacClassBHaltBeaconing( void )
{
    ...
    if( Ctx.BeaconCtx.Ctrl.BeaconMode == 1 )
    {
        ...
        LoRa_MacClassBEvents.Events.Beacon = 0;
...
{
#ifdef LORA_MAC_CLASSB_ENABLED
    if( LoRa_MacConfirmQueueIsCmdActive( MLME_PING_SLOT_INFO ) == true )
    {
        LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_OK, MLME_PING_SLOT_INFO );
...
uint8_t LoRa_MacClassBPingSlotChannelReq( uint8_t datarate, uint32_t frequency )
{
    ...
    if( frequency != 0 )
    {
        ...
        if( RegionVerify( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &verify, PHY_FREQUENCY ) == false )
        {
    ...
    verify.DatarateParams.DownlinkDwellTime = Ctx.LoRa_MacClassBParams.LoRa_MacParams->DownlinkDwellTime;

    if( RegionVerify( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &verify, PHY_RX_DR ) == false )
    {
...
void LoRa_MacClassBBeaconTimingAns( uint16_t beaconTimingDelay, uint8_t beaconTimingChannel, TimerTime_t lastRxDone )
{
    ...
    if( LoRa_MacConfirmQueueIsCmdActive( MLME_BEACON_TIMING ) == true )
    {
        if( Ctx.BeaconCtx.BeaconTimingDelay > CLASSB_BEACON_INTERVAL )
        {
            ...
            LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_BEACON_NOT_FOUND, MLME_BEACON_TIMING );
        }
        else
        {
            ...
            LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_OK, MLME_BEACON_TIMING );
        }

        Ctx.LoRa_MacClassBParams.MlmeConfirm->BeaconTimingDelay = Ctx.BeaconCtx.BeaconTimingDelay;
        Ctx.LoRa_MacClassBParams.MlmeConfirm->BeaconTimingChannel = Ctx.BeaconCtx.BeaconTimingChannel;
    }
#endif // LORA_MAC_CLASSB_ENABLED
}
...
void LoRa_MacClassBDeviceTimeAns( void )
{
    ...
    if( LoRa_MacConfirmQueueIsCmdActive( MLME_DEVICE_TIME ) == true )
    {
        if( currentTimeMs > SysTimeToMs( Ctx.BeaconCtx.NextBeaconRx ) )
        {
            ...
            Ctx.BeaconCtx.NextBeaconRx.Seconds = 0;
            Ctx.BeaconCtx.NextBeaconRx.SubSeconds = 0;
            LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_BEACON_NOT_FOUND, MLME_DEVICE_TIME );
        }
        else
        {
            Ctx.BeaconCtx.Ctrl.BeaconDelaySet = 1;
            ...
            Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
            LoRa_MacConfirmQueueSetStatus( LORA_MAC_EVENT_INFO_STATUS_OK, MLME_DEVICE_TIME );
        }
    }
#endif // LORA_MAC_CLASSB_ENABLED
}
...
bool LoRa_MacClassBBeaconFreqReq( uint32_t frequency )
{
    ...
    if( frequency != 0 )
    {
        ...
        if( RegionVerify( *Ctx.LoRa_MacClassBParams.LoRa_MacRegion, &verify, PHY_FREQUENCY ) == true )
        {
...
TimerTime_t LoRa_MacClassBIsUplinkCollision( TimerTime_t txTimeOnAir )
{
    ...
    beaconReserved = nextBeacon -
                     CLASSB_BEACON_GUARD -
                     Ctx.LoRa_MacClassBParams.LoRa_MacParams->ReceiveDelay1 -
                     Ctx.LoRa_MacClassBParams.LoRa_MacParams->ReceiveDelay2 -
                     txTimeOnAir;
...
void LoRa_MacClassBStopRxSlots( void )
{
    ...
    CRITICAL_SECTION_BEGIN( );
    LoRa_MacClassBEvents.Events.PingSlot = 0;
    LoRa_MacClassBEvents.Events.MulticastSlot = 0;
    CRITICAL_SECTION_END( );
#endif // LORA_MAC_CLASSB_ENABLED
}
...
{
#ifdef LORA_MAC_CLASSB_ENABLED
    LoRa_MacClassBEvents_t events;

    CRITICAL_SECTION_BEGIN( );
    events = LoRa_MacClassBEvents;
    LoRa_MacClassBEvents.Value = 0;
    CRITICAL_SECTION_END( );
    ...
    if( events.Value != 0 )
    {
        ...
        {
            LoRa_MacClassBProcessBeacon( );
        }
        if( events.Events.PingSlot == 1 )
        {
            LoRa_MacClassBProcessPingSlot( );
        }
        if( events.Events.MulticastSlot == 1 )
        {
            LoRa_MacClassBProcessMulticastSlot( );
        }
    }
#endif // LORA_MAC_CLASSB_ENABLED
}
