 ...
 */
#define CRYPTO_CTX_SIZE                 sizeof( LoRa_MacCryptoCtx_t )

/*
 * Size of the module non volatile context
 */
#define CRYPTO_NVM_CTX_SIZE             sizeof( LoRa_MacCryptoNvmCtx_t )
 ...
 */
#define CRYPTO_MIC_COMPUTATION_OFFSET   JOIN_REQ_TYPE_SIZE + LORA_MAC_JOIN_EUI_FIELD_SIZE + DEV_NONCE_SIZE + LORA_MAC_MHDR_FIELD_SIZE
...
/*
 * LoRa_Mac Crypto Non Volatile Context structure
 */
typedef struct sLoRa_MacCryptoNvmCtx
{
    /*
...
}LoRa_MacCryptoNvmCtx_t;

/*
 * LoRa_Mac Crypto Context structure
 */
typedef struct sLoRa_MacCryptoCtx
{
    /*
     ...
     */
    LoRa_MacCryptoNvmCtx_t* NvmCtx;
    /*
     * Callback function to notify the upper layer about context change
     */
    LoRa_MacCryptoNvmEvent EventCryptoNvmCtxChanged;
}LoRa_MacCryptoCtx_t;
 ...
 */
static LoRa_MacCryptoCtx_t CryptoCtx;

/*
 * Non volatile module context.
 */
static LoRa_MacCryptoNvmCtx_t NvmCryptoCtx;
 ...
 */
static LoRa_MacCryptoStatus_t PayloadEncrypt( uint8_t* buffer, int16_t size, KeyIdentifier_t keyID, uint32_t address, uint8_t dir, uint32_t frameCounter )
{
    if( buffer == 0 )
    {
    ...
    while( size > 0 )
    {
        ...
        {
            return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
    ...
    return LORA_MAC_CRYPTO_SUCCESS;
}
 ...
 * \retval                      - Status of the operation
 */
static LoRa_MacCryptoStatus_t FOptsEncrypt( uint16_t size, uint32_t address, uint8_t dir, FCntIdentifier_t fCntID, uint32_t frameCounter, uint8_t* buffer )
{
    if( buffer == 0 )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
    ...
    if( CryptoCtx.NvmCtx->LrWanVersion.Value > 0x01010000 )
    {
        ...
        switch( fCntID )
        {
            ...
            default:
                return LORA_MAC_CRYPTO_FAIL_PARAM;
        }
    }
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t PrepareB0( uint16_t msgLen, KeyIdentifier_t keyID, bool isAck, uint8_t dir, uint32_t devAddr, uint32_t fCnt, uint8_t* b0 )
{
    if( b0 == 0 )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t ComputeCmacB0( uint8_t* msg, uint16_t len, KeyIdentifier_t keyID, bool isAck, uint8_t dir, uint32_t devAddr, uint32_t fCnt, uint32_t* cmac )
{
    if( ( msg == 0 ) || ( cmac == 0 ) )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
    ...
    {
        return LORA_MAC_CRYPTO_ERROR_BUF_SIZE;
    }
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t VerifyCmacB0( uint8_t* msg, uint16_t len, KeyIdentifier_t keyID, bool isAck, uint8_t dir, uint32_t devAddr, uint32_t fCnt, uint32_t expectedCmac )
{
    if( msg == 0 )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
    ...
    {
        return LORA_MAC_CRYPTO_FAIL_MIC;
    }
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t PrepareB1( uint16_t msgLen, KeyIdentifier_t keyID, bool isAck, uint8_t txDr, uint8_t txCh, uint32_t devAddr, uint32_t fCntUp, uint8_t* b1 )
{
    if( b1 == 0 )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t ComputeCmacB1( uint8_t* msg, uint16_t len, KeyIdentifier_t keyID, bool isAck, uint8_t txDr, uint8_t txCh, uint32_t devAddr, uint32_t fCntUp, uint32_t* cmac )
{
    if( ( msg == 0 ) || ( cmac == 0 ) )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t GetKeyAddrItem( AddressIdentifier_t addrID, KeyAddr_t** item )
{
    ...
    return LORA_MAC_CRYPTO_ERROR_INVALID_ADDR_ID;
}
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t DeriveSessionKey10x( KeyIdentifier_t keyID, uint8_t* joinNonce, uint8_t* netID, uint8_t* devNonce )
{
    ...
    switch( keyID )
    {
        ...
        case APP_S_KEY:
            ...
            break;
        default:
            return LORA_MAC_CRYPTO_ERROR_INVALID_KEY_ID;
    }
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t DeriveSessionKey11x( KeyIdentifier_t keyID, uint8_t* joinNonce, uint8_t* joinEUI, uint8_t* devNonce )
{
 ...
 * \retval                    - Status of the operation
 */
static LoRa_MacCryptoStatus_t DeriveLifeTimeSessionKey( KeyIdentifier_t keyID, uint8_t* devEUI )
{
    if( devEUI == 0 )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
 ...
 * \retval                     - Status of the operation
 */
static LoRa_MacCryptoStatus_t GetLastFcntDown( FCntIdentifier_t fCntID, uint32_t* lastDown )
{
    if( lastDown == NULL )
    {
        return LORA_MAC_CRYPTO_ERROR_NPE;
...
{
    uint32_t lastDown = 0;
    if( GetLastFcntDown( fCntID, &lastDown ) != LORA_MAC_CRYPTO_SUCCESS )
    {
        return false;
    }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoInit( LoRa_MacCryptoNvmEvent cryptoNvmCtxChanged )
{
    ...
    memset1( (uint8_t*) CryptoCtx.NvmCtx, 0, sizeof( LoRa_MacCryptoNvmCtx_t ) );
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoGetFCntDown( FCntIdentifier_t fCntID, uint16_t maxFCntGap, uint32_t frameFcnt, uint32_t* currentDown )
{
    uint32_t lastDown = 0;
    int32_t fCntDiff = 0;
    LoRa_MacCryptoStatus_t cryptoStatus = LORA_MAC_CRYPTO_ERROR;
    ...
    if( cryptoStatus != LORA_MAC_CRYPTO_SUCCESS )
    {
        return cryptoStatus;
    }
    ...
    else
    {
        ...
        {  // Duplicate FCnt value, keep the current value.
            *currentDown = lastDown;
            return LORA_MAC_CRYPTO_FAIL_FCNT_DUPLICATED;
        }
        else
    ...
    if( CryptoCtx.NvmCtx->LrWanVersion.Fields.Minor == 0 )
    {
        ...
        {
            return LORA_MAC_CRYPTO_FAIL_MAX_GAP_FCNT;
        }
    }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoSetKey( KeyIdentifier_t keyID, uint8_t* key )
{
    ...
    if( ( keyID == GEN_APP_KEY ) || ( keyID == APP_KEY ) )
    {
        ...
        if( LoRa_MacCryptoDeriveMcRootKey( keyID ) != LORA_MAC_CRYPTO_SUCCESS )
        {
            return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
        if( LoRa_MacCryptoDeriveMcKEKey( MC_ROOT_KEY ) != LORA_MAC_CRYPTO_SUCCESS )
        {
            return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
    }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoPrepareJoinRequest( LoRa_MacMessageJoinRequest_t* macMsg )
{
    ...
    if( DeriveLifeTimeSessionKey( J_S_INT_KEY, macMsg->DevEUI ) != LORA_MAC_CRYPTO_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR;
    }
    if( DeriveLifeTimeSessionKey( J_S_ENC_KEY, macMsg->DevEUI ) != LORA_MAC_CRYPTO_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR;
    }
#endif
    ...
    // Serialize message
    if( LoRa_MacSerializerJoinRequest( macMsg ) != LORA_MAC_SERIALIZER_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SERIALIZER;
    }

    // Compute mic
    if( SecureElementComputeAesCmac( NULL, macMsg->Buffer, ( LORA_MAC_JOIN_REQ_MSG_SIZE - LORA_MAC_MIC_FIELD_SIZE ), micComputationKeyID, &macMsg->MIC ) != SECURE_ELEMENT_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoPrepareReJoinType1( LoRa_MacMessageReJoinType1_t* macMsg )
{
    ...
    {
        return LORA_MAC_CRYPTO_ERROR_RJCOUNT1_OVERFLOW;
    }

    // Serialize message
    if( LoRa_MacSerializerReJoinType1( macMsg ) != LORA_MAC_SERIALIZER_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SERIALIZER;
    }
    ...
    if( SecureElementComputeAesCmac( NULL, macMsg->Buffer, ( LORA_MAC_RE_JOIN_1_MSG_SIZE - LORA_MAC_MIC_FIELD_SIZE ), J_S_INT_KEY, &macMsg->MIC ) != SECURE_ELEMENT_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoPrepareReJoinType0or2( LoRa_MacMessageReJoinType0or2_t* macMsg )
{
    ...
    {
        return LORA_MAC_CRYPTO_FAIL_RJCOUNT0_OVERFLOW;
    }

    // Serialize message
    if( LoRa_MacSerializerReJoinType0or2( macMsg ) != LORA_MAC_SERIALIZER_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SERIALIZER;
    }
    ...
    if( SecureElementComputeAesCmac( NULL, macMsg->Buffer, ( LORA_MAC_RE_JOIN_0_2_MSG_SIZE - LORA_MAC_MIC_FIELD_SIZE ), S_NWK_S_INT_KEY, &macMsg->MIC ) != SECURE_ELEMENT_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoHandleJoinAccept( JoinReqIdentifier_t joinReqType, uint8_t* joinEUI, LoRa_MacMessageJoinAccept_t* macMsg )
{
    ...
    LoRa_MacCryptoStatus_t retval = LORA_MAC_CRYPTO_ERROR;
    ...
    if( SecureElementAesEncrypt( macMsg->Buffer + LORA_MAC_MHDR_FIELD_SIZE, ( macMsg->BufSize - LORA_MAC_MHDR_FIELD_SIZE ), encryptionKeyID, ( procBuffer + micComputationOffset ) ) != SECURE_ELEMENT_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
    ...
    memcpy1( macMsg->Buffer + LORA_MAC_MHDR_FIELD_SIZE, ( procBuffer + micComputationOffset ), ( macMsg->BufSize - LORA_MAC_MHDR_FIELD_SIZE ) );

    // Parse the message
    if( LoRa_MacParserJoinAccept( macMsg ) != LORA_MAC_PARSER_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_PARSER;
    }
    ...
    if( CryptoCtx.NvmCtx->LrWanVersion.Fields.Minor == 0 )
    {
        ...
        if( SecureElementVerifyAesCmac( macMsg->Buffer, ( macMsg->BufSize - LORA_MAC_MIC_FIELD_SIZE ), macMsg->MIC, micComputationKeyID ) != SECURE_ELEMENT_SUCCESS )
        {
            return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
    }
    else
    {
        ...
        memcpyr( &procBuffer[bufItr], joinEUI, LORA_MAC_JOIN_EUI_FIELD_SIZE );
        bufItr += LORA_MAC_JOIN_EUI_FIELD_SIZE;
        ...
        if( SecureElementVerifyAesCmac( procBuffer,  ( macMsg->BufSize + micComputationOffset - LORA_MAC_MHDR_FIELD_SIZE - LORA_MAC_MIC_FIELD_SIZE ), macMsg->MIC, micComputationKeyID ) != SECURE_ELEMENT_SUCCESS )
        {
            return LORA_MAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
        }
        ...
        {
            return LORA_MAC_CRYPTO_FAIL_JOIN_NONCE;
        }
    }
    ...
    {
        // Derive lifetime keys
        retval = LoRa_MacCryptoDeriveMcRootKey( APP_KEY );
        if( retval != LORA_MAC_CRYPTO_SUCCESS )
        {
            return retval;
        }

        retval = LoRa_MacCryptoDeriveMcKEKey( MC_ROOT_KEY );
        if( retval != LORA_MAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
    ...
    {
        // prior LoRaWAN 1.1.0
        retval = LoRa_MacCryptoDeriveMcRootKey( GEN_APP_KEY );
        if( retval != LORA_MAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoSecureMessage( uint32_t fCntUp, uint8_t txDr, uint8_t txCh, LoRa_MacMessageData_t* macMsg )
{
    ...
    {
        return LORA_MAC_CRYPTO_FAIL_FCNT_SMALLER;
    }
    ...
    // Serialize message
    if( LoRa_MacSerializerData( macMsg ) != LORA_MAC_SERIALIZER_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_SERIALIZER;
    }
    ...
    if( CryptoCtx.NvmCtx->LrWanVersion.Fields.Minor == 1 )
    {
        ...
        retval = ComputeCmacB1( macMsg->Buffer, ( macMsg->BufSize - LORA_MAC_MIC_FIELD_SIZE ), S_NWK_S_INT_KEY, macMsg->FHDR.FCtrl.Bits.Ack, txDr, txCh, macMsg->FHDR.DevAddr, fCntUp, &cmacS );
        if( retval != LORA_MAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
        //cmacF = aes128_cmac(FNwkSIntKey, B0 | msg)
        retval = ComputeCmacB0( macMsg->Buffer, ( macMsg->BufSize - LORA_MAC_MIC_FIELD_SIZE ), F_NWK_S_INT_KEY, macMsg->FHDR.FCtrl.Bits.Ack, UPLINK, macMsg->FHDR.DevAddr, fCntUp, &cmacF );
        if( retval != LORA_MAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
...
#endif
    {
        ...
        retval = ComputeCmacB0( macMsg->Buffer, ( macMsg->BufSize - LORA_MAC_MIC_FIELD_SIZE ), NWK_S_ENC_KEY, false, UPLINK, macMsg->FHDR.DevAddr, fCntUp, &macMsg->MIC );
        if( retval != LORA_MAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
...
LoRa_MacCryptoStatus_t LoRa_MacCryptoUnsecureMessage( AddressIdentifier_t addrID, uint32_t address, FCntIdentifier_t fCntID, uint32_t fCntDown, LoRa_MacMessageData_t* macMsg )
{
    ...
    // Parse the message
    if( LoRa_MacParserData( macMsg ) != LORA_MAC_PARSER_SUCCESS )
    {
        return LORA_MAC_CRYPTO_ERROR_PARSER;
    }
    ...
    {
        return LORA_MAC_CRYPTO_FAIL_ADDRESS;
    }
    ...
    // Verify mic
    retval = VerifyCmacB0( macMsg->Buffer, ( macMsg->BufSize - LORA_MAC_MIC_FIELD_SIZE ), micComputationKeyID, isAck, DOWNLINK, address, fCntDown, macMsg->MIC );
    if( retval != LORA_MAC_CRYPTO_SUCCESS )
    {
        return retval;
    }
